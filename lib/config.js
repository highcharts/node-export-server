...
const { mergeConfigOptions } = require('./utils.js');

const { promptsConfig } = require('./schemas/config.js');
...
const loadConfigFile = async (options) => {
  try {
    const configFile = options.customCode.loadConfig;
    ...
    log(
      1,
      `[config] - Unable to load config from the ${configFile}: ${error}.`
    );
  }
};
...
/**
 * Display a prompt for the manual configuration.
 */
