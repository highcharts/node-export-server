 ...
 * object, based on the `getCopy` flag. The `newOptions` object can be strictly
 * validated depending on the `strictCheck` flag.
 *
...
function _loadConfigFile(cliArgs) {
  ...
  const loadConfig = configIndex > -1 && cliArgs[configIndex + 1];

  // Check if the `--loadConfig` is present and has a correct value
  if (loadConfig && allowFileResources) {
    try {
      // Load an optional custom JSON config file
      return isAllowedConfig(
        readFileSync(getAbsolutePath(loadConfig), 'utf8'),
        false,
        allowCodeExecution
      );
    } catch (error) {
      logWithStack(
        2,
        error,
        `[config] Unable to load the configuration from the ${loadConfig} file.`
      );
    }
  }
