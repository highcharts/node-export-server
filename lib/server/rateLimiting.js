 ...
 * @param {Object} limitingConfig - Configuration options for rate limiting.
 */
export default (app, limitingConfig) => {
  const msg =
    'Too many requests, you have been rate limited. Please try again later.';

  // Options for the rate limiter
  const rateOptions = {
    max: limitingConfig.maxRequests || 30,
    window: limitingConfig.window || 1,
    delay: limitingConfig.delay || 0,
    trustProxy: limitingConfig.trustProxy || false,
    skipKey: limitingConfig.skipKey || false,
    skipToken: limitingConfig.skipToken || false
  };
