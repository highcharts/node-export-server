...
/**
 * @overview Manages WebSocket connections and interactions for the Highcharts
 * Export Server. Supports client reconnection, message sending, and telemetry
 * data transmission.
 */
 ...
 */
export function webSocketInit(address) {
  webSocketOptions = getOptions().webSocket;
  if (webSocketOptions.enable === true) {
    // Get the secret directly from envs
    const webSocketSecret = envs.WEB_SOCKET_SECRET;
    ...
    const connectionOptions = {
      ...
      headers: {
        // Set an access token
        auth: jwt.sign({ success: 'success' }, webSocketSecret, {
          algorithm: 'HS256'
        }),
    ...
    if (webSocketOptions.messageInterval > 0) {
      _sendingMessageInterval(webSocketOptions);
    }
  }
}
 ...
 */
export function connect(webSocketUrl, connectionOptions, clientOptions) {
  ...
  webSocketClient.on('close', (code) => {
    ...
    _reconnect(webSocketUrl, connectionOptions, clientOptions);
  });
 ...
 */
function _sendingMessageInterval(webSocketOptions) {
 ...
 * @param {Object} clientOptions - Options for WebSocket client.
 */
function _reconnect(webSocketUrl, connectionOptions, clientOptions) {
  if (
