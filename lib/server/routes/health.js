...
export default function addHealthRoutes(app) {
  ...
  app.get('/health/export-stats', (_, res) => {
    log(
      4,
      '[health.js] GET /health/export-stats [200] - returning export stats.'
    );
    ...
    res.send({
      ...
      performedExports: pool.processedWorkCount(),
      failedExports: pool.droppedWork(),
      svgExportAttempts: pool.svgExportAttempts(),
      jsonExportAttempts: pool.processedWorkCount() - pool.svgExportAttempts()
    });
  });
}
