...
const config = {
  ...
  /**
   * The `instr` validator that returns a Zod schema with an optional stricter
   * check based on the `strictCheck` parameter.
   *
   * The validation schema ensures the same work as the `object` validator.
   *
   * @param {boolean} strictCheck - Determines if stricter validation should be
   * applied.
   *
  ...
  /**
   * The `options` validator that returns a Zod schema with an optional stricter
   * check based on the `strictCheck` parameter.
   *
   * The validation schema ensures the same work as the `object` validator.
   *
   ...
   * check based on the `strictCheck` parameter.
   *
   * The validation schema ensures the same work as a nullable `positiveNum`
   * validator.
   *
   * @param {boolean} strictCheck - Determines if stricter validation should be
   * applied.
   ...
   */
  height(strictCheck) {
    return v.positiveNum(strictCheck).nullable();
  },
  ...
  /**
   * The `globalOptions` validator that returns a Zod schema with an optional
   * stricter check based on the `strictCheck` parameter.
   *
   * The validation schema ensures the same work as the `object` validator.
   *
  ...
  /**
   * The `themeOptions` validator that returns a Zod schema with an optional
   * stricter check based on the `strictCheck` parameter.
   *
   * The validation schema ensures the same work as the `object` validator.
   *
  ...
  /**
   * The `svg` validator that returns a Zod schema with an optional stricter
   * check based on the `strictCheck` parameter.
   *
   * The validation schema ensures the same work as the `string` validator.
   *
    ...
    return strictCheck
      ? z
          ...
          .refine(
            ...
            {
              params: {
                errorMessage: `The value must be a string that contains '<svg' or '<?xml' and could contain a forbidden value`
              }
            }
          )
      : z
...
const ExportSchema = (strictCheck) =>
  z
    .object({
      ...
      globalOptions: config.globalOptions(false),
      themeOptions: config.themeOptions(false),
