 ...
 */
const customErrorMap = (issue, context) => {
...
const v = {
   ...
   * the forbidden values: 'false', 'NaN', 'undefined', 'null', and ''.
   *
   * - When `strictCheck` is false, the schema will accept trimmed strings and
   * null. The forbidden values: 'false', 'NaN', 'undefined', 'null', and ''
  ...
  string(strictCheck) {
    return strictCheck
      ? z
          ...
          .refine(
            (value) =>
              !['false', 'NaN', 'undefined', 'null', ''].includes(value),
            {
              params: {
   ...
   * values.
   */
  stringArray(filterCallback, strictCheck) {
...
const PuppeteerSchema = (strictCheck) =>
  z
    .object({
      args: v.stringArray(
        (value) => !['false', 'undefined', 'NaN', 'null', ''].includes(value),
        strictCheck
      )
    })
    .partial();
...
const HighchartsSchema = (strictCheck) =>
  z
    .object({
      ...
      coreScripts: v.stringArray(
        (value) => scriptsNames.core.includes(value),
        strictCheck
      ),
      moduleScripts: v.stringArray(
        (value) => scriptsNames.modules.includes(value),
        strictCheck
      ),
      indicatorScripts: v.stringArray(
        (value) => scriptsNames.indicators.includes(value),
        strictCheck
      ),
      customScripts: v.stringArray(
...
const PayloadSchema = (strictCheck) =>
  z
    .object({
      svg: v.string(false),
      b64: v.boolean(strictCheck),
      noDownload: v.boolean(strictCheck),
      requestId: v.string(false)
    })
    .partial();

// Schema for the final config
export const ConfigSchema = z.object({
  ...
  payload: PayloadSchema(true)
});

// Schema for the cli config
export const CliSchema = z.object({
  ...
  payload: PayloadSchema(false)
});

// Schema for the request body config
export const RequestSchema = z.object({
  infile: v.object(false),
  options: v.object(false),
  data: v.object(false),
  svg: v.string(false),
  type: v.enum(['jpeg', 'jpg', 'png', 'pdf', 'svg'], false),
  constr: v.enum(['chart', 'stockChart', 'mapChart', 'ganttChart'], false),
  height: v.positiveNum(false),
  width: v.positiveNum(false),
  scale: c.scale(false),
  globalOptions: v.object(false),
  themeOptions: v.object(false),
  resources: c.resources(false),
  callback: v.string(false),
  customCode: v.string(false),
  b64: v.boolean(false),
  noDownload: v.boolean(false)
});

// Schema for the environment variables config
export const EnvSchema = z.object({
  // puppeteer
  PUPPETEER_ARGS: v.stringArray(
    (value) => !['false', 'undefined', 'NaN', 'null', ''].includes(value),
    false
  ),
  ...
  HIGHCHARTS_CORE_SCRIPTS: v.stringArray(
    (value) => scriptsNames.core.includes(value),
    false
  ),
  HIGHCHARTS_MODULE_SCRIPTS: v.stringArray(
    (value) => scriptsNames.modules.includes(value),
    false
  ),
  HIGHCHARTS_INDICATOR_SCRIPTS: v.stringArray(
    (value) => scriptsNames.indicators.includes(value),
    false
  ),
  HIGHCHARTS_CUSTOM_SCRIPTS: v.stringArray(
    (value) => value.startsWith('https://') || value.startsWith('http://'),
    false
  ),
...
/**
 * Validates the final configuration options using the ConfigSchema.
 *
...
/**
 * Validates the CLI options using the CliSchema.
 *
 * @param {object} configOptions - The configuration options from CLI to
 * validate.
 *
 * @returns {object} - The parsed and validated configuration options.
 */
...
/**
 * Validates the request body options using the PayloadSchema.
 *
 * @param {object} configOptions - The configuration options from request body
 * to validate.
 *
 * @returns {object} - The parsed and validated configuration options.
 */
export const validateRequest = (configOptions) =>
  RequestSchema.partial().parse(configOptions);
