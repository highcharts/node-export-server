...
const { fixType } = require('./utils.js');
const { log } = require('./logger');

const doExport = (options, chartJson, endCallback) => {
  let {
  ...
  // Prepare outfile's name and location
  const outfile =
    exportOptions.outfile ||
    `${exportOptions.tempDir}chart.${fixType(exportOptions.type)}`;
  ...
  if (customCodeOptions && !customCodeOptions.allowCodeExecution) {
    if (
      customCodeOptions.callback ||
      customCodeOptions.resources ||
      (payloadOptions && payloadOptions.customCode)
    ) {
    ...
    if (payloadOptions) {
      payloadOptions.customCode = false;
    }
  }

  // Guess export type based on file extension
  if (!exportOptions.type) {
    exportOptions.type = outfile.split('.').pop();
  }
  ...
  // Check if resources are directly placed in the string
  if (
    customCodeOptions.resources &&
    typeof customCodeOptions.resources === 'string'
  ) {
    try {
      customCodeOptions.resources = JSON.parse(customCodeOptions.resources);
    } catch (error) {
      log(2, `[chart] - Error parsing resources: ${error}.`);
      return endCallback(
        false,
        'Error parsing resources, check your JSON syntax.'
      );
    }
  }

  // Check if resources contain reference to files
  if (customCodeOptions.resources && customCodeOptions.resources.files) {
    try {
      customCodeOptions.resources.files = customCodeOptions.resources.files.split(
        ','
      );
    } catch (error) {
      log(2, `[chart] - Error organizing resource files: ${error}.`);
      return endCallback(
        false,
  ...
  let customCode = (payloadOptions && payloadOptions.customcode) || false;
  if (customCode && typeof exportOptions.instr !== 'string') {
    try {
      ...
      log(
        1,
        '[chart] - The customCode contains invalid data: must be a function; either stringified or pure.'
      );
      customCode = false;
    }
  }

  /// TO DO: Decide where to put the themeOptions
  // if (
  //   exportOptions &&
  ...
  //     log(
  //       1,
  //       '[chart] - Theme options not found.'
  //     );
  //   }
  // }
  ///

  /// TO DO: Decide where to put the styledMode and themeOptions
  // {
  //   styledMode: exportOptions.styledMode,
  //   themeOptions: exportOptions.themeOptions || false
  // }
  ///
  ...
  const executeExport = () => {
    ...
    pool
      .postWork(chartJson || exportOptions.str, options)
  ...
  if (
    ...
    readFile(customCodeOptions.callback, (error, data) => {
      if (error) {
...
const doStraightInject = (options) => {
  ...
  if (typeof exportOptions.instr !== 'undefined') {
    exportOptions.str = exportOptions.instr.replace(/\t/g, ' ').trim();

    // Get rid of the ;
    if (exportOptions.str[exportOptions.str.length - 1] === ';') {
      exportOptions.str = exportOptions.str.substring(
        0,
    ...
    return (
      ...
      endCallback(
        ...
        'Malformed input: Please make sure that your JSON/JavaScript options' +
          ' are sent using the "options" attribute, and that if you\'re using SVG it\'s unescaped.'
      )
    );
  }
};
...
const exportAsString = (stringToExport, options, endCallback) => {
  ...
  if (exportOptions.forceInject) {
    return doStraightInject(options);
  }
  ...
  try {
    ...
    // Error message
    const message = `Only JSON configurations and SVG is allowed for this server.
 ...
 * @param endCallback {function} - The function to call when exporting is done
 */
module.exports = async (options, endCallback) => {
  ...
  if (exportOptions.infile && exportOptions.infile.length) {
    ...
    return readFile(exportOptions.infile, (error, infile) => {
      if (error) {
        return log(0, `[chart] - Error loading input file:${error}.`);
      }

      // Get the string representation
      options.export.instr = infile.toString();
