...
const findChartSize = (options) => {
  const { chart, exporting } = options.export?.options || options.export?.instr;

  let globalChart;
  try {
    globalChart = JSON.parse(options.export?.globalOptions)?.chart;
  } catch (error) {
    globalChart = false;
  }
  ...
  return {
    ...
    width:
      ...
      globalChart.width ||
      600,
    scale: options.export?.scale || exporting.scale || 1
  };
};
...
const doExport = (options, chartJson, endCallback, svg) => {
  ...
  // In case of straight inject, chart's size was set earlier
  if (!exportOptions.strInj && options) {
    // Find a correct chart size scale
    options.export = {
      ...options.export,
