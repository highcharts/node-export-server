...
import { getGeneralOptions } from './config.js';
...
/**
 * Starts an export process. The `options` contains final options gathered
 ...
 *
 * @param {Object} [options=getGeneralOptions()] - The options object containing
 * configuration for a custom export.
 ...
 */
export async function startExport(options = getGeneralOptions(), endCallback) {
  ...
  if (exportOptions.infile !== null) {
    try {
      ...
      exportOptions.instr = readFileSync(exportOptions.infile, 'utf8');
  ...
  // Export using options from the raw representation
 ...
 */
export async function singleExport(options = getGeneralOptions()) {
 ...
 */
export async function batchExport(options = getGeneralOptions()) {
 ...
 *
 * @returns {boolean} The value of allowCodeExecution.
 */
 ...
 * `_prepareExport` function with the updated options.
 *
...
function _doStraightInject(options, endCallback) {
  try {
    ...
    // Call the `_prepareExport` with an straight injected string
...
function _exportAsString(stringToExport, options, endCallback) {
  ...
  if (
    ...
    // Call the `_prepareExport` with an SVG string
  ...
  try {
    ...
    if (options.customLogic.allowCodeExecution) {
      ...
      // Do not allow straight injection the allowCodeExecution is false
      return endCallback(
        new ExportError(
...
async function _prepareExport(options, endCallback, json, svg) {
  ...
  // Prepare custom logic (custom code, callback, resources) options
  try {
  ...
  // Prepare a correct chart's type
  exportOptions.type = fixType(exportOptions.type, exportOptions.outfile);

  // Prepare `chart` and `exporting` properties to keep it lean and mean
  if (json) {
  ...
  // Set the width to null in case of exporting to an SVG
  if (exportOptions.type === 'svg') {
    exportOptions.width = null;
  }

  // Prepare size and scale
  options.export = {
    ...exportOptions,
...
/**
 * Calculates the size and scale for chart exports based on the provided export
 * options.
 *
 ...
 *
 * @param {Object} exportOptions - The options object containing export
 * configuration.
 *
 * @returns {Object} An object containing the calculated `height`, `width`, and
 * `scale` values for the chart export.
 */
 ...
 * custom code, and callback. If code execution is allowed, it processes the
 * custom logic options accordingly. If code execution is not allowed, it
 * disables the usage of resources, custom code, and callback.
 *
 * @param {Object} customLogicOptions - Options containing custom logic settings
 * such as resources, custom code, and callback.
 * @param {boolean} allowCodeExecution - A flag indicating whether code
 * execution is allowed.
 *
 * @throws {ExportError} Throws an error if code execution is not allowed but
 * custom logic options are still provided.
 */
function _handleCustomLogic(customLogicOptions, allowCodeExecution) {
  ...
  if (allowCodeExecution) {
    ...
    if (
      ...
      // The allowFileResources is always set to false for HTTP requests to
      // avoid injecting arbitrary files from the fs
    ...
    // Check if there are any custom code
    if (customLogicOptions.customCode === null) {
      log(3, `[cli] No custom code found.`);
    }

    // Check if there are any custom code
    if (customLogicOptions.callback === null) {
      log(3, `[cli] No callback found.`);
    }

    // Check if there are any resources
    if (customLogicOptions.resources === null) {
      log(3, `[cli] No resources found.`);
    }
  } else {
    // If the allowCodeExecution flag is not set, we should refuse the usage of
    // callback, resources, and custom code. Additionally, the worker will
 ...
 * files.
 * @param {boolean} allowCodeExecution - Whether to allow executing arbitrary JS
 * code.
 *
...
/**
 * Handles the loading and validation of `globalOptions` and `themeOptions` in
 * the export options. If the option is a string and references a JSON file
 ...
 * this process, the option is set to null.
 *
 * @param {Object} exportOptions - The export options containing `globalOptions`
 * and `themeOptions`.
 * @param {boolean} allowFileResources - A flag indicating whether file
 * resources (e.g., .json files) are allowed.
 * @param {boolean} allowCodeExecution - A flag indicating whether executing
 * code (e.g., within JSON or theme options) is allowed.
 *
 * @throws {Error} If there is an error loading or parsing `globalOptions` or
 * `themeOptions`, the error is logged and the option is set to null.
 */
function _handleGlobalAndTheme(
  ...
  // Check globalOptions and themeOptions
