...
const doExport = (options, chartJson, endCallback, svg) => {
  ...
  // The customCode function
  let customCodeFunc = (payloadOptions && payloadOptions.customCode) || false;
  try {
    if (customCodeFunc && typeof customCodeFunc === 'string') {
      customCodeFunc = customCodeFunc.trim();

      if (customCodeFunc.indexOf('function') !== 0) {
        customCodeFunc = `(options) => {${customCodeFunc.toString().trim()}}`;
      }
    } else {
      customCodeFunc = false;
    }
  } catch (notice) {
    log(
      1,
  ...
  payloadOptions.customCode = customCodeFunc;
