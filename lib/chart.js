...
const doExport = (options, chartJson, endCallback, svg) => {
  ...
  try {
    if (exportOptions && exportOptions.globalOptions) {
      if (exportOptions.globalOptions.endsWith('.json')) {
        exportOptions.globalOptions = isCorrectJSON(
          readFileSync(
            path.join(__basedir, exportOptions.globalOptions),
            'utf8'
          )
        );
      } else {
    ...
    exportOptions.globalOptions = {};
    log(1, '[chart] - Global options not found.');
  }

  // Prepare theme options
  try {
    if (exportOptions && exportOptions.themeOptions) {
      if (exportOptions.themeOptions.endsWith('.json')) {
        exportOptions.themeOptions = isCorrectJSON(
          readFileSync(path.join(__basedir, exportOptions.themeOptions), 'utf8')
        );
      } else {
        exportOptions.themeOptions = isCorrectJSON(
          exportOptions.themeOptions,
          true
        );
      }
    }
    ...
    exportOptions.themeOptions = {};
    log(1, '[chart] - Theme options not found.');
  }
