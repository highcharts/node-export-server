...
/**
 * @overview This module provides a manual configuration feature that prompts
 * users to customize and save their desired settings into a configuration file.
 */
 ...
 *
 * @param {string} createConfig - The name of the configuration file to save to.
    ...
    await prompts(allQuestions, {
      ...
      onSubmit: async (prompt, answer) => {
        ...
        if (++questionsCounter === allQuestions.length) {
          try {
            // Validate the prompt result
            configFile = validateOptions(configFile);
            ...
            logWithStack(
              1,
              error,
              `[prompt] An error occurred while creating the ${createConfig} file.`
            );
          }
          return true;
        }
