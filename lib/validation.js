...
Copyright (c) 2016-2026, Highsoft
...
const v = {
  ...
  /**
   * The `path` validator that returns a Zod schema with an optional stricter
   * check based on the `strictCheck` parameter.
   *
   * The validation schema ensures that:
   *
   * - When `strictCheck` is true, the schema will accept path directory string
   * values.
   *
   * - When `strictCheck` is false, the schema will accept path directory regex
   * string values, null, and trimmed string values that can either be
   * 'undefined', 'null', ''. It will transform the string to a path directory
   * string, or to null if it is 'undefined', 'null', or ''.
   *
   * @function nonNegativeNum
   *
   * @param {boolean} path - Determines if stricter validation should
   * be applied.
   *
   * @returns {z.ZodSchema} A Zod schema object for validating path directory
   * string values.
   */
  path(strictCheck) {
    return strictCheck
...
export const validators = {
  ...
  /**
   * The `tempDir` validator that returns a Zod schema with an optional stricter
   * check based on the `strictCheck` parameter.
   *
   ...
   * be applied.
   *
   * @returns {z.ZodSchema} A Zod schema object for validating the `tempDir`
   * option.
   */
  tempDir(strictCheck) {
  ...
  /**
   * The `useNpm` validator that returns a Zod schema with an optional
   * stricter check based on the `strictCheck` parameter.
   *
   ...
   * be applied.
   *
   * @returns {z.ZodSchema} A Zod schema object for validating the `useNpm`
   * option.
   */
  useNpm(strictCheck) {
  ...
  /**
   * The `maxUploadSize` validator that returns a Zod schema with an optional
   * stricter check based on the `strictCheck` parameter.
   *
   * The validation schema ensures the same work as the `positiveNum` validator.
   *
   ...
   * `maxUploadSize` option.
   */
  maxUploadSize(strictCheck) {
    return v.positiveNum(strictCheck);
  },
  ...
  /**
   * The `proxyUsername` validator that returns a Zod schema with an optional
   * stricter check based on the `strictCheck` parameter.
   *
   * The validation schema ensures the same work as the `string` validator.
   *
   ...
   */
  proxyUsername(strictCheck) {
    return v.string(strictCheck);
  },

  /**
   * The `proxyPassword` validator that returns a Zod schema with an optional
   * stricter check based on the `strictCheck` parameter.
   *
   * The validation schema ensures the same work as the `string` validator.
   *
  ...
  /**
   * The `allowXLink` validator that returns a Zod schema with an optional
   * stricter check based on the `strictCheck` parameter.
   *
   * The validation schema ensures the same work as the `boolean` validator.
   *
   ...
   * be applied.
   *
   * @returns {z.ZodSchema} A Zod schema object for validating the `allowXLink`
   * option.
   */
  allowXLink(strictCheck) {
    return v.boolean(strictCheck);
  },

  /**
   * The `excludeClasses` validator that returns a Zod schema with an optional
   * stricter check based on the `strictCheck` parameter.
   *
   * The validation schema ensures the same work as the `stringArray` validator.
   *
   * @function excludeClasses
   *
   * @param {boolean} strictCheck - Determines if stricter validation should
   * be applied.
   *
   * @returns {z.ZodSchema} A Zod schema object for validating the
   * `excludeClasses` option.
   */
  excludeClasses(strictCheck) {
    return v.stringArray(
      (value) => !['undefined', 'null', ''].includes(value),
      ',',
      strictCheck
...
const PuppeteerSchema = (strictCheck) =>
  z
    .object({
      ...
      tempDir: validators.tempDir(strictCheck)
    })
    .partial();
...
const HighchartsSchema = (strictCheck) =>
  z
    .object({
      ...
      useNpm: validators.useNpm(strictCheck),
...
const ProxySchema = (strictCheck) =>
  z
    .object({
      ...
      username: validators.proxyUsername(false),
      password: validators.proxyPassword(false),
...
const ServerSchema = (strictCheck) =>
  z.object({
    ...
    maxUploadSize: validators.maxUploadSize(strictCheck).optional(),
...
const OtherSchema = (strictCheck) =>
  z
    .object({
      ...
      allowXLink: validators.allowXLink(strictCheck)
    })
    .partial();
...
export const EnvSchema = z.object({
  ...
  PUPPETEER_TEMP_DIR: validators.tempDir(false),

  // highcharts
  HIGHCHARTS_VERSION: validators.version(false),
  HIGHCHARTS_CDN_URL: validators.cdnUrl(false),
  HIGHCHARTS_USE_NPM: validators.useNpm(false),
  ...
  SERVER_MAX_UPLOAD_SIZE: validators.maxUploadSize(false),
  ...
  SERVER_PROXY_USERNAME: validators.proxyUsername(false),
  SERVER_PROXY_PASSWORD: validators.proxyPassword(false),
  ...
  OTHER_ALLOW_XLINK: validators.allowXLink(false),
  OTHER_EXCLUDE_CLASSES: validators.excludeClasses(false),
 ...
 *
 * @param {z.ZodIssueBase} issue - The issue object representing the validation
 * error.
