 ...
 * instances, creating and clearing pages, injecting custom resources,
 * and setting up Highcharts for server-side rendering. The module ensures that
 * resources are correctly managed and can handle failures during operations
 * like launching the browser or creating new pages.
 */
 ...
 *
 * @param {Array.<string>} [puppeteerArg=[]] - Additional arguments
 ...
 * If the browser instance is not available, returns false. The function creates
 * a new page, disables caching, sets content using `_setPageContent()`,
 * and returns the created Puppeteer page.
 *
 * @async
 * @function newPage
 *
 ...
 *
 * @returns {(boolean|object)} Returns false if the browser instance
 * is not available, or a Puppeteer Page object representing the newly created
 * page.
 *
 * @throws {ExportError} Throws an `ExportError` if no valid browser
 ...
 * thrown error if clearing the page content fails.
 *
 * @async
 * @function clearPage
 *
