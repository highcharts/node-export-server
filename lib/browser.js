...
import { __projDir, getAbsolutePath } from './utils.js';

import ExportError from './errors/ExportError.js';

// Get the template for pages
const pageTemplate = readFileSync(
  join(__projDir, 'templates', 'template.html'),
  'utf8'
);
...
export async function createBrowser(puppeteerArgs) {
  // Get `debug`, `other` and `puppeteer` options
  const { debug, other, puppeteer } = getOptions();
  ...
  const launchOptions = {
    ...
    userDataDir: getAbsolutePath(puppeteer.tempDir || join(__projDir, 'tmp')),
    args: puppeteerArgs || [],
    handleSIGINT: false,
    handleSIGTERM: false,
    handleSIGHUP: false,
  ...
  // Create a browser
  if (!browser) {
    // The maximum number of retries to open a browser instance
    const maxTries = 25;
 ...
 *
 * @throws {ExportError} Throws an `ExportError` if no valid browser has been
 * connected or if a page is invalid or closed.
 */
