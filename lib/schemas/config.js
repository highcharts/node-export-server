...
const defaultConfig = {
  ...
  server: {
    ...
    port: {
      value: +process.env.HIGHCHARTS_SERVER_PORT || 8080,
      type: 'number',
      description: 'A port of a server. Defaults to 8080.'
    },
    ssl: {
      enable: {
  ...
  pool: {
    initialWorkers: {
      value: +process.env.HIGHCHARTS_POOL_MIN_WORKERS || 4,
      type: 'number',
      description: 'The number of initial workers to spawn.'
    },
    maxWorkers: {
      value: +process.env.HIGHCHARTS_POOL_MAX_WORKERS || 4,
      type: 'number',
  ...
  logging: {
    level: {
      value: +process.env.HIGHCHARTS_LOG_LEVEL || 4,
      type: 'number',
      cliName: 'logLevel',
      description:
