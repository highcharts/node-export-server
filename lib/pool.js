 ...
 */
const init = (config) => {
  log(3, '[pool.js] - Initializing pool.');

  if (pool) {
    return log(
      4,
      '[pool.js] - Already initialized, please kill it before creating a new one.'
    );
  }
 ...
 */
const killAll = async () => {
  log(3, '[pool.js] - Killing all workers.');
...
const postWork = async (chart) => {
  log(3, '[pool.js] - Work received, starting to process.');

  if (!pool) {
    log(1, '[pool.js] - Work received, but pool has not been started.');
  ...
  if (browserHandle.workCount > poolConfig.workLimit && poolConfig.workLimit) {
    log(4, `[pool.js] - Worker ${browserHandle.id} reached limit, restarting.`);
  ...
  try {
    ...
    // Perform an export
    const result = await doExport(browserHandle.browser, poolConfig, chart);

    // Save the end time
    let workEnd = new Date().getTime();

    log(4, `[pool.js] - Work completed in ${workEnd - workStart}ms.`);
