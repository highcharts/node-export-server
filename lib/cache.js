...
const updateCache = async (config) => {
  try {
    ...
    await fsPromises.writeFile(join(cachePath, 'sources.js'), cache.sources);
  } catch (e) {
...
const checkCache = async (config) => {
  ...
  if (fs.existsSync(join(cachePath, 'manifest.json'))) {
    const manifest = require(join(cachePath, 'manifest.json'));
    ...
    if (requestUpdate) {
      ...
      // Load the sources
      cache.sources = await fsPromises.readFile(
        join(cachePath, 'sources.js'),
        'utf8'
      );
      extractVersion();
    }
