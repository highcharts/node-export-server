 ...
 * @returns {string} The fetched script content.
 */
const fetchAndProcessScript = async (
  ...
  if (shouldThrowError) {
    throw new Error(
      `Could not fetch the ${script}.js. The script might not exist in the requested version.`
    );
  } else {
    log(
      2,
...
const updateCache = async (config, sourcePath) => {
  ...
  try {
    ...
    log(1, `[cache] Error fetching Highcharts sources: ${error}`);
  }
};
