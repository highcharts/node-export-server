...
const saveConfigToManifest = async (config) => {
  ...
  try {
    ...
    log(1, `[cache.js] - Error writing cache manifest: ${error}.`);
  }
};
...
const checkCache = async (config) => {
  ...
  if (existsSync(manifestPath)) {
    ...
    if (manifest.version !== config.version) {
      log(
        3,
        '[cache.js] - Highcharts version mismatch in cache, need to re-fetch.'
      );
      requestUpdate = true;
    } else if (
      ...
      log(
        3,
        '[cache.js] - The cache modules and requested modules does not match, need to re-fetch..'
      );
      requestUpdate = true;
    } else {
      ...
      requestUpdate = config.modules.some((moduleName) => {
        if (!manifest.modules[moduleName]) {
          log(
            3,
            `[cache.js] - The ${moduleName} missing in cache, need to re-fetch.`
          );
          return true;
        }
      });
