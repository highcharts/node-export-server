...
export async function checkAndUpdateCache(
  ...
  // Get the cache path
  const cachePath = join(__dirname, highchartsOptions.cachePath);
...
export function getCachePath() {
  return join(__dirname, getGeneralOptions().highcharts.cachePath);
}
 ...
 * the cache manifest.
 */
async function _saveConfigToManifest(config, fetchedModules = {}) {
  const newManifest = {
...
async function _fetchScripts(
  ...
  const requestOptions = proxyAgent
    ? {
        agent: proxyAgent,
        timeout: getGeneralOptions().server.proxy.timeout
      }
    : {};
