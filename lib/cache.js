...
import { __projDir, createDir, getAbsolutePath } from './utils.js';
...
export async function checkCache(highchartsOptions, serverProxyOptions) {
  try {
    ...
    createDir(cachePath, { recursive: true });
...
async function _updateCache(highchartsOptions, serverProxyOptions, sourcePath) {
  try {
    ...
    let cdnUrl = highchartsOptions.cdnUrl || cache.cdnUrl;
    cdnUrl = cdnUrl.endsWith('/') ? cdnUrl.slice(0, -1) : cdnUrl;
...
async function _fetchScript(
  ...
  if (useNpm) {
    try {
      // Correct path to NPM script
      script = join('node_modules', 'highcharts', script);

      // Log fetched script
      log(4, `[cache] Fetching script from NPM - ${script}`);

      // Fetch the script from NPM
      response = readFileSync(join(__projDir, script), 'utf8');

      // If OK, return its text representation
      if (fetchedModules && response) {
        fetchedModules[_extractModuleName(script)] = 1;
      }
      return response;
    } catch {
      // Proceed
    ...
    response = await get(script, requestOptions);
...
function _configureRequest(serverProxyOptions) {
  // Get the `host`, `port`, `username` and `password` from the proxy options
  const { host, port, username, password } = serverProxyOptions;
...
function _extractHcVersion(cacheSources) {
  ...
  return hcVersion.includes('Highcharts JS') ? hcVersion : 'unknown';
}
