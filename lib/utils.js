...
const optionsStringify = (options, allowFunctions) => {
  const replacerCallback = (n, value) => {
    if (typeof value === 'string') {
      ...
      if (
        ...
        value = `EXP_FUN${(value + '').replaceAll(/\n|\t|\r/g, ' ')}EXP_FUN`;
      }
    }
  ...
  // Add the replacer if allowed
  if (allowFunctions) {
    stringifyArgs.push(replacerCallback);
  }

  // Stringify options and if required, replace special functions marks
  return JSON.stringify(...stringifyArgs).replaceAll(/"EXP_FUN|EXP_FUN"/g, '');
};
