...
/**
 * Adjusts the constructor name by transforming and normalizing it based on
 * common chart types.
 *
 ...
 *
 * @param {unknown} data - The data to be validated and parsed as JSON. Must be
 * either an object or a string.
 * @param {boolean} [toString=false] - Whether to return a stringified JSON
 * version of the parsed data.
 * @param {boolean} [allowFunctions=false] - Whether to allow functions in the
 * parsed JSON. If true, functions are preserved.
 *
 * @returns {(Object|string|null)} Returns the parsed JSON object,
 * a stringified JSON object if `toString` is true, or null if the data is not
 * valid JSON or parsing fails.
 */
 ...
 *
 * @returns {Function} A function to calculate the elapsed time
 * in milliseconds.
 */
 ...
 *
 * In order for a function to be preserved, it needs to follow the format
 * `function (...) {...}`. It can also be stringified.
 *
 ...
 * in the output.
 * @param {boolean} stringifyFunctions - If set to true, functions are saved as
 * strings.
 *
