...
export const __projDir = resolve(dirname(fileURLToPath(import.meta.url)), '..');

// The /public directory path
export const __publicDir = resolve(__projDir, 'public');

// The package.json file
export const packageJson = JSON.parse(
  readFileSync(resolve(__projDir, 'package.json'), 'utf8')
);
 ...
 *
 * @param {string} svgString - The SVG string to which add the xlink namespace.
 *
 * @returns {string} The SVG string with the xlink namespace added if it was
 * not present.
 */
 ...
 * @param {Object} [options={}] - Optional options for directory creation. These
 * options are passed directly to the `fs.mkdirSync` method.
 */
export function createDir(dirPath, options = {}) {
  !existsSync(dirPath) && mkdirSync(dirPath, options);
}
...
export function getAbsolutePath(path) {
  // First, normalize the path to clean any pollution
  const normalizedPath = normalize(path);

  // Then, check if the path is absolute or relative and return the correct one
