...
export default async (page, chart, options) => {
  ...
  try {
    ...
    const viewportHeight = Math.abs(Math.ceil(size.chartHeight || exportOptions.height));
    const viewportWidth = Math.abs(Math.ceil(size.chartWidth || exportOptions.width));
