...
module.exports = async (browser, chart, options) => {
  try {
    ...
    if (typeof chart !== 'string') {
      chartWidth = parseFloat(chart.chart.width || options.export.width) || 600;
      chartHeight =
        parseFloat(chart.chart.height || options.export.height) || 400;
    }

    // Decide whether display error or debbuger wrapper around it
    const displayErrors =
      options.export?.options?.chart?.displayErrors &&
