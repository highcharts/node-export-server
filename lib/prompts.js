 ...
 */
export async function manualConfig(configFileName) {
   ...
   *
   * @param {Object} _ - Unused, automatically passed argument.
   * @param {Array} categories - The selected categories to configure.
   *
   * @returns {Promise<boolean>} Resolves to `true` once the configuration
   * process is completed and saved.
   */
  const onSubmit = async (_, categories) => {
    ...
    await prompts(allQuestions, {
       ...
       * @param {any} answer - The user's response to the prompt.
       *
       * @returns {Promise<void>} Resolves once the answer is processed.
       */
...
/**
 * Prepares a prompt configuration object based on the provided entry and
 * section. The prompt configuration includes the prompt type, initial value,
 * format, and additional options for rendering and validation in interactive
 * prompts.
 *
 * @param {Array} entry - An array where the first element is the name of the
 * option, and the second element is an object containing the option details.
 * @param {string} section - The section name for the prompt, used to categorize
 * the prompt message.
 *
 * @returns {Object} The prepared prompt object containing configuration for the
 * interactive prompt, including the type, initial value, and any formatting or
 * choices required.
 */
 ...
 *
 * @param {Object} objectToUpdate - The object in which nested properties will
 ...
 * property.
 *
 * @returns {Object} The updated object with the specified value assigned to the
 * nested property.
 */
