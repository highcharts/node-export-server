import { EnvConfig } from '../../lib/envs';
...
describe('Environment variables should be correctly parsed', () => {
  test('HIGHCHARTS_VERSION accepts latests and not unrelated strings', () => {
    ...
    expect(() => EnvConfig.partial().parse(env)).toThrow();

    env.HIGHCHARTS_VERSION = 'latest';
    expect(EnvConfig.partial().parse(env).HIGHCHARTS_VERSION).toEqual('latest');
  });

  test('HIGHCHARTS_VERSION accepts proper version strings like XX.YY.ZZ', () => {
    const env = { HIGHCHARTS_VERSION: '11' };
    expect(EnvConfig.partial().parse(env).HIGHCHARTS_VERSION).toEqual('11');

    env.HIGHCHARTS_VERSION = '11.0.0';
    expect(EnvConfig.partial().parse(env).HIGHCHARTS_VERSION).toEqual('11.0.0');

    env.HIGHCHARTS_VERSION = '9.1';
    expect(EnvConfig.partial().parse(env).HIGHCHARTS_VERSION).toEqual('9.1');
  ...
  test('HIGHCHARTS_CDN_URL should start with http:// or https://', () => {
    ...
    expect(EnvConfig.partial().parse(env).HIGHCHARTS_CDN_URL).toEqual(
      'http://example.com'
    );
  ...
  test('CORE, MODULE, INDICATOR scripts should be arrays', () => {
    ...
    const parsed = EnvConfig.partial().parse(env);
  ...
  test('HIGHCHARTS_FORCE_FETCH should be a boolean', () => {
    ...
    expect(EnvConfig.partial().parse(env).HIGHCHARTS_FORCE_FETCH).toEqual(true);

    env.HIGHCHARTS_FORCE_FETCH = 'false';
    expect(EnvConfig.partial().parse(env).HIGHCHARTS_FORCE_FETCH).toEqual(
      false
    );
  });
});
