...
- Fixed the `PUPPETEER_TEMP_DIR` validation message never being shown. An empty options object was passed where the message should have gone, so an invalid value was rejected with a generic error rather than the explanation that had been written for it.
